...
7. Archive the leg: `$TEAM_HOME/bin/archive-turn.sh <pane> <role> "<!-- leg:...>"` — dumps scrollback and trims it to **this leg only** (from the leg marker to the end), writes `state/<session>/turns/`, prints the path. The body never enters your context; extract only the ```` ```handoff ```` block from the archive (tail/grep). Loop.
