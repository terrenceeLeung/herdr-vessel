...
4. Inject the task with `herdr pane run <pane> "<leg marker>\n<task>"`, where the leg marker is a first-line `<!-- leg:<UTC-ts> -->` unique to this leg (used to trim the archive in step 7). The task text carries: what to do this leg, the previous handoff's what/why/artifacts/next_action, the previous turn's archive path (“上一棒完整输出在 <path>，需要细节自己读”), and a reminder to end with a ```` ```handoff ```` block per the contract.
...
7. Archive the leg: `$TEAM_HOME/bin/archive-turn.sh <pane> <role> "<!-- leg:...>"` — trims to **this leg only** and prints the path. Two fidelity layers, automatic: **exact extraction from the pi session file (structured messages, verbatim)** when available, terminal scrollback as fallback. The body never enters your context; extract only the ```` ```handoff ```` block from the archive (tail/grep). Loop.
