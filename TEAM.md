...
7. **存整棒**：`$TEAM_HOME/bin/archive-turn.sh <pane> <role> "<!-- leg:...>"` ——抓 scrollback 并按 leg 标记裁剪出**本棒**内容（不含历史），落盘 `state/<session>/turns/` 并打印路径。全程不进你的上下文；handoff block 从存档文件里提取（tail/grep），只有 block 本身进上下文。
