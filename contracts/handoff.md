...
1. 读 `$TEAM_HOME/state/<herdr-session>/outbox/<role>/latest.md`（先校验 mtime ≥ 本棒注入时间；陈旧 → 升船长）；
2. 从文件中提取最后一个 ` ```handoff ` block，按上面规则校验，并核对 `to` 在 TEAM.md 路由表的允许下游内；
...
4. 等它重发（`wait agent-status ... idle`），重新校验。同一棒累计两次无效 → 升船长。
