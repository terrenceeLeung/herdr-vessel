...
│   ├── orchestrator.sh        ← 调度器启动器（自带环境，防漏 export）
...
```bash
~/projects/herdr-vessel/bin/orchestrator.sh
```

一条命令搞定（自带 `TEAM_HOME` / `TEAM_ROLE=orchestrator` 环境、调度 prompt、herdr skill）——不要再手动 export，之前漏设环境变量会导致调度器静默残废（无 prompt、无 skill、插件 no-op）。`TEAM_ROLE=orchestrator` 会激活执法插件（F001：注入门禁 + 自动记帐 + hop 熔断 + 你的输入自动 reset）。角色 pane 由 `team-up.sh` 自动带上各自的 `TEAM_ROLE`。紧急禁用插件：`HERDR_VESSEL_EXT=off`。
