...
```bash
export TEAM_HOME=~/projects/herdr-vessel TEAM_ROLE=orchestrator
pi --append-system-prompt "$(cat $TEAM_HOME/orchestrator/PROMPT.md)" \
   --skill "$TEAM_HOME/orchestrator/skills/herdr"
```

`TEAM_ROLE=orchestrator` 会激活执法插件（F001：注入门禁 + 自动记帐 + hop 熔断 + 你的输入自动 reset）。角色 pane 由 `team-up.sh` 自动带上各自的 `TEAM_ROLE`。紧急禁用插件：`HERDR_VESSEL_EXT=off`。
